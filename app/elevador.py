...
# -*- coding: utf-8 -*-
"""
Elevador GUI (COM apenas) - versão compacta
-------------------------------------------
- Mesma lógica de comunicação do elevador.py original.
...
class RealTimePlots:
    def __init__(self, parent):
        ...
        # Altura um pouco maior, mas ainda cabendo em 1366x768
        height = 1.9
        ...
        # margens internas: evita cortar rótulos e aproveita bem o espaço
...
class ElevadorGUI:
    def __init__(self, master: tk.Tk):
        ...
        # janela mais compacta para 1366x768
        ...
        ttk.Button(top, text="Atualizar",
                   command=self._update_com_list).pack(side="left", padx=4)
        self.btn_connect = ttk.Button(top, text="Conectar",
                                      command=self._toggle_connection)
        self.btn_connect.pack(side="left", padx=8)

        # Status
        self.var_status = tk.StringVar(value="Desconectado")
        ttk.Label(master, textvariable=self.var_status,
                  relief="sunken", anchor="w").pack(fill="x", padx=6, pady=2)

        # Indicadores
        indic = ttk.Frame(master)
        indic.pack(fill="x", padx=6, pady=4)
        self.var_andar = tk.StringVar(value="-")
        self.var_dest  = tk.StringVar(value="-")
        self.var_motor = tk.StringVar(value="-")
        self.var_pos   = tk.StringVar(value="-")
        self.var_vel   = tk.StringVar(value="-")
        ...
        mk("Andar (A)",  self.var_andar)
        mk("Destino (D)", self.var_dest)
        mk("Motor (M)",  self.var_motor)
        mk("Pos (mm)",   self.var_pos)
        mk("Vel (mm/s)", self.var_vel)
        mk("Temp (°C)",  self.var_temp)

        # Envio $OD\r
        sendf = ttk.LabelFrame(master, text="Enviar $OD\\r (O=0..3 D=0..3)")
        sendf.pack(fill="x", padx=6, pady=6)
        self.var_origem = tk.IntVar(value=0)
        self.var_destino = tk.IntVar(value=1)
        ttk.Label(sendf, text="Origem").pack(side="left", padx=4)
        ttk.Spinbox(sendf, from_=0, to=3,
                    textvariable=self.var_origem,
                    width=4).pack(side="left")
        ttk.Label(sendf, text="Destino").pack(side="left", padx=4)
        ttk.Spinbox(sendf, from_=0, to=3,
                    textvariable=self.var_destino,
                    width=4).pack(side="left")
        ttk.Button(sendf, text="Enviar $OD",
                   command=self._enviar_od).pack(side="left", padx=8)
        for d in range(4):
            ttk.Button(sendf, text=f"-> {d}",
                       command=lambda dd=d: self._enviar_rapido(dd)
                       ).pack(side="left", padx=2)

        # Logging CSV
        logf = ttk.Frame(master)
        logf.pack(fill="x", padx=6, pady=4)
        ttk.Checkbutton(logf, text="Gravar CSV",
                        variable=self.logging_enabled,
                        command=self._toggle_csv).pack(side="left")
        self.lbl_csv = ttk.Label(logf, text="")
        self.lbl_csv.pack(side="left", padx=8)

        # Plots
        self.plots = RealTimePlots(master)

        # Timers (IMPORTANTE para comunicação – não mexer)
    ...
    def _connect(self):
        ...
        if not port:
            messagebox.showwarning(
                "Porta COM",
                "Nenhuma COM selecionada. Clique em Atualizar para listar as COM disponíveis."
            )
            return
        try:
            # Mesma configuração do código original que funciona
    ...
    def _enviar_od(self):
        if not self.ser:
            ...
            return
        o = int(self.var_origem.get())
        d = int(self.var_destino.get())
        if not (0 <= o <= 3 and 0 <= d <= 3):
            messagebox.showwarning(
                "Valores",
                "Origem/Destino devem estar entre 0 e 3."
            )
            return
    ...
    def _toggle_csv(self):
        if self.logging_enabled.get():
            ...
            try:
                ...
                self.csv_writer.writerow(
                    ["timestamp", "A", "D", "M", "pos_mm", "vel_mms", "temp_C"]
                )
            except Exception as e:
...
def main():
    ...
    # Ícone da UnB, se disponível
    try:
        root.iconbitmap("unb.ico")
    except Exception as e:
